...
export function historyEntryToFeedItem(entry: HistoryEntry): FeedItem {
  ...
  const mediaType = historyFeedType(entry);
  ...
  return {
    ...
    resolutions: resolution
      ? [{ type: resolution, status: 'FINISH', url: entry.resultUrl, name: resolution }]
      : [{ type: mediaType, status: 'FINISH', url: entry.resultUrl }],
  };
}
