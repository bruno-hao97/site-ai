          ...
          >
            ...
            <div className="project-pick-list">
              ...
              {filtered.length === 0 && !query.trim() && (
                <p className="project-pick-empty">{t('project.picker.empty')}</p>
              )}
            </div>

            {query.trim() && !exactMatch && (
              <button type="button" className="project-pick-create" onClick={handleCreate}>
                <Plus size={14} /> {t('project.picker.create', { name: query.trim() })}
              </button>
            )}
            ...
            {current && (
              ...
              >
                <X size={14} /> {t('project.picker.removeFrom', { name: current.name })}
              </button>
            )}
          </div>,
          document.body,
